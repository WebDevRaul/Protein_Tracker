...
const validateCalc = ({ age, gender, activity, height, weight }) => {
  ...
  // Validate Age
  if (Validator.isInt(age, {min:-Infinity, max:17})) {
    errors.age = 'Minimum age must be 18';
  }
  if (Validator.isInt(age, {min:100, max:Infinity})) {
    errors.age = 'Miximum age required is 99';
  }
  // Validate Height
  if (Validator.isInt(height, {min:-Infinity, max:0})) {
    errors.height = 'Minimum height required is 1';
  }
  if (Validator.isInt(height, {min:301, max: Infinity})) {
    errors.height = 'Miximum height required is 300';
  }
  // Validate Weight
  if (Validator.isInt(weight, {min:-Infinity, max:0})) {
    errors.weight = 'Minimum weight required is 1';
  }
  if (Validator.isInt(weight, {min:1001, max: Infinity})) {
  ...
  if (!Validator.isInt(age, {allow_leading_zeroes: false})) {
    errors.age = 'Only valid numbers allowed';
  }
  if (!Validator.isInt(height, {allow_leading_zeroes: false})) {
    errors.height = 'Only valid numbers allowed';
  }
  if (!Validator.isInt(weight, {allow_leading_zeroes: false})) {
